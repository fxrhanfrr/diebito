...
import { Send, Bot, User, Sparkles, PlusCircle, MessageSquare } from 'lucide-react';
...
import { getAIChatHistory, saveAIChatHistory, getAIChatSessions, createAIChatSession } from '@/lib/firestore';
...
export default function AIAssistant() {
  ...
  useEffect(() => {
    if (user && !initialLoaded) {
      getAIChatSessions(user.id).then((fetchedSessions) => {
        if (fetchedSessions.length > 0) {
          setSessions(fetchedSessions);
          setCurrentSessionId(fetchedSessions[0].id);
        } else {
          // Create initial session
          createAIChatSession(user.id, "New Chat").then(id => {
            setCurrentSessionId(id);
            setSessions([{ id, title: "New Chat", updatedAt: new Date() }]);
  ...
  useEffect(() => {
    if (user && currentSessionId) {
      getAIChatHistory(user.id, currentSessionId).then((history) => {
        setMessages(history);
        setInitialLoaded(true); // Ensures it's true after first real load
      }).catch(err => {
  ...
  useEffect(() => {
    if (user && initialLoaded && currentSessionId && messages.length > 0) {
      const timer = setTimeout(() => {
        ...
        saveAIChatHistory(user.id, messages, currentSessionId, title).catch(err => console.error("Failed to save history", err));
      }, 1000);
      return () => clearTimeout(timer);
    }
  }, [messages, user, initialLoaded, currentSessionId]);

  const handleNewChat = async () => {
    if (!user) return;
    setMessages([]); // Clear visually
    const id = await createAIChatSession(user.id, "New Chat");
    setCurrentSessionId(id);
    setSessions(prev => [{ id, title: "New Chat", updatedAt: new Date() }, ...prev]);
  };
  ...
  return (
    <AuthGuard>
      <div className="flex flex-row h-[calc(100vh-4rem)] bg-gray-50">
        
        {/* Sidebar */}
        <div className="w-64 bg-white border-r flex-shrink-0 hidden md:flex flex-col">
          <div className="p-4 border-b">
            <Button className="w-full flex items-center justify-start gap-2" onClick={handleNewChat}>
          ...
          <div className="p-4 bg-white border-t">
            <div className="max-w-3xl mx-auto">
              <form onSubmit={handleSubmit} className="flex gap-2">
                ...
                />
                <Button type="submit" disabled={isLoading || !input || !currentSessionId}>
